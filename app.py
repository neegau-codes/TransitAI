...
def search_routes():
    ...
    Supports both V2 JSON contract (with 'query') and legacy structured searches (with 'source', 'destination').
    """
    data = request.json or {}

    # Check for V2 natural-language query input
    if "query" in data and isinstance(data["query"], str) and data["query"].strip():
        ...
        if origin and destination:
            ...
            constraints = {
                "budget_limit": v2_intent.get("budget"),
                "arrival_deadline": v2_intent.get("arrive_before"),
                "excluded_modes": parsed_params.get("additional_constraints", {}).get("excluded_modes", [])
            }
            routes_res = route_engine.find_routes(origin, destination, dep_time, constraints=constraints)
            if "error" not in routes_res:
